...
function getCritRateValues(stackCount: number): readonly number[] {
  return ROYAL_LONGSWORD_CRIT_RATE_PER_STACK.map((value) => value * stackCount)
}

function createStackEffect(stackCount: (typeof stackCounts)[number]): CombatActionEffect {
  return {
    activation: "active",
    exclusivity: { group: "royal-longsword-focus", variant: `${stackCount}-stack` },
    id: `weapon.royal-longsword.focus.${stackCount}-stack.crit-rate`,
    label: `宗室长剑 · 本次命中前${stackCount}层专注暴击率`,
    source: { kind: "weapon", weaponId: "RoyalLongsword" },
    target: "critRate",
