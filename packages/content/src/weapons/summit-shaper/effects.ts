...
export const SUMMIT_SHAPER_ATTACK_PERCENT_PER_STACK = [0.04, 0.05, 0.06, 0.07, 0.08] as const
...
function getAttackPercentValues(stackCount: number, shielded: boolean): readonly number[] {
  ...
  return SUMMIT_SHAPER_ATTACK_PERCENT_PER_STACK.map((value) => value * multiplier)
}
...
function createStackEffect(
  ...
  return {
    ...
    exclusivity: { group: "summit-shaper-golden-majesty", variant: `${state}-${stackCount}-stack` },
    id: `weapon.summit-shaper.golden-majesty.${state}.${stackCount}-stack.attack-percent`,
    label: `斫峰之刃 · ${stateLabel}${stackCount}层攻击力`,
...
/** Typed selected shield-state and stack contributions of Summit Shaper. */
export const summitShaperCombatActionEffects: readonly CombatActionEffect[] = [
