...
function renderSupportEquipmentRegistry(category: "healing" | "recipient"): GeneratedFile {
  const typeName = category === "healing" ? "HealingEquipmentEffect" : "RecipientEquipmentEffect"
  const suffix = `${typeName}s`
  const entities: EntityExport[] = []
  for (const [directory, kind] of [["artifacts", "artifact"], ["weapons", "weapon"]] as const) {
    for (const slug of listEntitySlugs(directory)) {
      const path = join(sourceRoot, directory, slug, "support-effects.ts")
      if (!existsSync(path)) continue
      for (const exportName of listExportedConstants(path).filter((name) => name.endsWith(suffix))) {
        entities.push({ exportName, kind, modulePath: `../${directory}/${slug}/support-effects.js`, slug })
      }
    }
  }
  return {
    path: join(registryRoot, `equipment-${category}-effects.generated.ts`),
    content: [
      "// Generated by tools/generate-registries.ts; do not edit by hand.",
      `import type { ${typeName} } from "../rules/equipment/${category}/types.js"`,
      "",
      ...entities.map((entity) => `import { ${entity.exportName} } from "${entity.modulePath}"`),
      "",
      `export const ${category}EquipmentEffects: readonly ${typeName}[] = [`,
      entities.map((entity) => `  ...${entity.exportName}`).join(",\n"),
      "]", ""
    ].join("\n")
  }
}

function renderEquipmentCapabilityRegistry(): GeneratedFile {
  const entities = listEntitySlugs("weapons").flatMap((slug) => {
    const path = join(sourceRoot, "weapons", slug, "effects.ts")
    return listExportedConstants(path).filter((name) => name.endsWith("CombatCapabilities"))
      .map((exportName) => ({ slug, exportName }))
  })
  return {
    path: join(registryRoot, "equipment-capabilities.generated.ts"),
    content: [
      "// Generated by tools/generate-registries.ts; do not edit by hand.",
      'import type { CombatEquipmentCapability } from "../combat/capabilities.js"', "",
      ...entities.map(({ slug, exportName }) => `import { ${exportName} } from "../weapons/${slug}/effects.js"`), "",
      "export const equipmentCombatCapabilities: readonly CombatEquipmentCapability[] = [",
      entities.map(({ exportName }) => `  ...${exportName}`).join(",\n"), "]", ""
    ].join("\n")
  }
}
...
const generatedFiles = [
  renderEquipmentCapabilityRegistry(),
  renderCharacterCatalogRegistry(),
  renderCharacterCombatRegistry(),
  renderEquipmentActionEffectRegistry(),
  renderSupportEquipmentRegistry("healing"),
  renderSupportEquipmentRegistry("recipient"),
